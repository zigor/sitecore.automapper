...
namespace Sitecore.AutoMapper.Mappers.Fields
{
  ...
  public class CustomFieldMapper<TSource> : CustomFieldMapper where TSource : CustomField
  {
    /// <summary>
    ...
    {
      return SetMemberValue(field, field.Value, destination, context);
    }
    ...
    /// <param name="context">The context.</param>
    /// <returns></returns>
    public static TDestination SetMemberValue<TDestination>(TSource field, object value, TDestination destination,
      ResolutionContext context)
    {
      var member = GetPropertyMember(field.InnerField.Name, destination, context);
      ...
      RunAfterMap(field, destination, member);
